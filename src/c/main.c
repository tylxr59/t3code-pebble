...
static const char *prv_title_for_selection(void) {
  ...
  if ((s_view == VIEW_MESSAGES || s_view == VIEW_EXPANDED) && s_message_count > 0) {
    return s_messages[s_selected].role;
  }
  return "";
}
...
static void prv_render(void) {
  ...
  if (s_view == VIEW_EXPANDED) {
    ...
    if (s_view == VIEW_PROJECTS) {
      snprintf(footer, sizeof(footer), "Project %d/%d  SEL threads", s_selected + 1, count);
    } else if (s_view == VIEW_THREADS) {
      snprintf(footer, sizeof(footer), "Thread %d/%d  SEL msgs", s_selected + 1, count);
    } else {
...
static void prv_up_click(ClickRecognizerRef recognizer, void *context) {
  ...
  if (s_view == VIEW_EXPANDED) {
    ...
    int count = prv_current_count();
    if (count > 0) s_selected = (s_selected + count - 1) % count;
  }
  prv_render();
}
...
static void prv_down_click(ClickRecognizerRef recognizer, void *context) {
  ...
  if (s_view == VIEW_EXPANDED) {
    ...
    int count = prv_current_count();
    if (count > 0) s_selected = (s_selected + 1) % count;
  }
  prv_render();
}
...
static void prv_window_load(Window *window) {
  ...
  s_header_layer = text_layer_create(GRect(0, 0, bounds.size.w, 28));
  ...
  text_layer_set_background_color(s_footer_layer, GColorLightGray);
